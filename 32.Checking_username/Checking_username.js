...
let current_user = ["FareEha", "MuNeeb", "AsAd", "FahAd", "HumA"];

// Make another list of five usernames called new_users. Make sure one or two of the new usernames are also in the current_users list.
let new_user = ["FareEha", "Saba", "UsamA", "Admin", "BiLal"];

new_user.forEach((newUsername) => {
  let lowerCase = newUsername.toLowerCase();
  if (current_user.map((c_user) => c_user.toLowerCase()).includes(lowerCase)) {
    console.log(
      `The user name ${newUsername} is not available. Please write a different username. `
    );
  } else {
