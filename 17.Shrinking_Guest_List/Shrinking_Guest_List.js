...
// ysha per mainne 6 guest k array ko print karwaya hai
...
// sorry guest for not unviting
console.log('\nSorry we can not arrange big table , only two peoples will be invited.');

// yahan per main ne guest remove kiye hain
while (Guest_List.length > 2) {
    let remove_Guest = Guest_List.pop();
    console.log(`Sorry Mr. ${remove_Guest}, You are not invited to the dinner `);
}

// or bachey hue 2 hi bulaye 
for (let i = 0; i < Guest_List.length; i++) {
    console.log('Dear Mr./Ms. ' + Guest_List[i] + '\n\n You are still inivited at the party. \n\n Thank You!');
}
Guest_List.splice(0, 2);
// yani 0 se 2 wlaey delet hojayengy baqi sarey array se delete hogaye , 
